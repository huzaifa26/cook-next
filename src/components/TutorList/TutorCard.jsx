...
export default function TutorCard({ tutor }) {
  ...
  return (
    <div className='w-full md:w-[88.383vw] sm:w-[88.383vw] border-2 border-[rgba(255,219,184,1)] px-[29.5px] py-[45px] rounded-[16px] flex sm:flex-col xsm:flex-col xsm:items-center gap-[46px] sm:gap-[5.509vw] items-start'>
      <Link className='rounded-full' href={'/profile?data='+encodeURIComponent(JSON.stringify(tutor))}>
        <Image className='min-w-[154px] max-w-[154px] rounded-full' width={154} height={154} loader={() => tutor?.image} src={tutor?.image} alt="" />
      </Link>
      <div>
        <div>
          <h3 className='font-rubik font-semibold text-[32px] leading-[37.92px] tracking-[-0.02em]'>{tutor.name}</h3>
          <div className='flex items-center gap-x-[18px] gap-y-[8px] sm:flex-col sm:items-start sm:mt-[8px]'>
            <div className='flex items-center gap-[7px]'>
              ...
              <p className='font-outfit font-normal text-[18px] leading-[22.68px] text-primary2'>{tutor?.subject}</p>
            </div>
            <div className='flex items-center gap-[7px]'>
              <svg className='max-w-[17px] max-h-[17px]' width="100" height="100" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className='font-outfit font-normal text-[18px] leading-[22.68px] text-primary2'>15 active students</p>
            </div>
          </div>
        </div>

        <div className='min-w-[35.347vw] xsm:w-full'>
          <p className='font-outfit font-normal text-[16px] leading-[24px] mt-[30px]'>{tutor?.introduction}</p>
          ...
          <Link href={'/profile?data='+encodeURIComponent(JSON.stringify(tutor))}>
      ...
      <div className='xsm:w-full'>
        <div className='flex items-center justify-center gap-[20px]'>
          ...
          <div className='flex flex-col justify-center items-center'>
            <p className='font-rubik font-bold text-[24px] leading-[28.44px]'>{`$${tutor?.price}`}</p>
