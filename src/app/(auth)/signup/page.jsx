...
export default function Page() {
  ...
  const formSubmitHandler = async (e) => {
    ...
    if (data.password.trim().length < 8) {
      setPasswordError("Password must consist of atleast 8 characters");
      return
    } else {
  ...
  return (
    <main className='flex relative md:min-h-[1194px] sm:min-h-[1194px] xsm:h-[844px]'>
      <div className='blurSignup h-auto md:min-h-[1194px] sm:min-h-[1194px] bg-primary2 xsm:hidden w-[49.306vw] md:w-full sm:w-full min-h-[846px] flex gap-[13px] justify-center overflow-hidden xsm:relative xsm:z-20'>
      ...
      <div className='bg-[white] xsm:z-20 xsm:w-full md:border-2 md:border-[rgba(255,219,184,1)] md:rounded-[23px] md:h-fit sm:border-2 sm:border-[rgba(255,219,184,1)] sm:rounded-[23px] sm:h-fit w-[50.694vw] md:w-[80.24vw] sm:w-[80.24vw] h-full flex flex-col items-center pt-[53px] md:absolute sm:absolute xsm:absolute md:left-[50%] md:translate-x-[-50%] md:top-[50%] md:translate-y-[-50%] sm:left-[50%] sm:translate-x-[-50%] sm:top-[50%] sm:translate-y-[-50%]'>
        <div className='w-fit xsm:w-[90vw] h-fit flex flex-col justify-center items-center'>
          ...
          <div className='w-full flex flex-col gap-[8px] mt-[24px]'>
            <button  onClick={()=> googleSigninHandler()} className='xsm:w-full flex hover:bg-primaryLighten2 transition-all duration-200 items-center justify-center gap-[14px] w-[322px] h-[56px] border border-[rgba(255,219,184,1)] font-outfit font-normal text-[20px] leading-[30px]'>
              <Image src={Google} alt="" />
              Continue with Google
            </button>
            <button onClick={()=> facebookSigninHandler()} className='xsm:w-full flex hover:bg-primaryLighten2 transition-all duration-200 items-center justify-center gap-[14px] w-[322px] h-[56px] border border-[rgba(255,219,184,1)] font-outfit font-normal text-[20px] leading-[30px]'>
        ...
        <div className='relative my-[40px] w-full flex justify-center items-center'>
        ...
        <div>
          ...
          <form onSubmit={formSubmitHandler} className='w-[346px] xsm:w-[90vw] flex flex-col gap-[18px] items-center mt-[29px]'>
            ...
            <div className='w-full flex flex-col gap-[2px]'>
              ...
              <div className='pr-[10px] flex items-center gap-[10px] outline-primary2 overflow-hidden border border-[rgba(255,219,184,1)] h-[37px]'>
                ...
                {!showPassword ?
                  <svg onClick={() => setShowPassword(true)} className='cursor-pointer' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 12C1 12 5 4 12 4C19 4 23 12 23 12C23 12 19 20 12 20C5 20 1 12 1 12Z" stroke="#D17721" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z" stroke="#D17721" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                  :
              ...
              {
                error &&
                <p className='text-primary2 font-outfit leading-normal text-[14px] text-right'>{error}</p>
              }
            </div>
          ...
          <Link href='/signin'>
            <p className='text-primary2 font-rubik text-[18px] font-medium tracking-[-0.36px] mb-[33px] md:mb-[690px] sm:mb-[69px] text-center mt-[29px]'>Have an account? Log in</p>
          </Link>
        </div>
      </div>
    </main>
